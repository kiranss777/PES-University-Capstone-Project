...
if __name__ == "__main__":
    ...
    # Initialize the Generator and Discriminator on CPU
    gen = Generator(Z_DIM, IN_CHANNELS, img_channels=3).cpu()
    critic = Discriminator(Z_DIM, IN_CHANNELS, img_channels=3).cpu()

    for img_size in [4, 8, 16, 32, 64, 128, 256, 512, 1024]:
        num_steps = int(log2(img_size / 4))
        x = torch.randn((1, Z_DIM, 1, 1)).cpu()  # Move input tensor to CPU
